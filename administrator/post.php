...
if(isset($_GET['f_id'])) {
		...
		$id=$_GET["f_id"];
		echo $id;
		$approve=mysql_query("update forum set approve='Yes' where f_id='$id'");
...
<body>
	...
	<div class="col-md-8">
		<?php
			if($result2){
		echo"Post Title :".$one["title"]."<br>"; 
		echo"Post :".$one["post"]."<br>";
		echo"replies :".$one["reply"]."<br>";
		echo"<form action=\"forum_manager.php\" method=\"post\"><button class=\"btn btn-success btn-large pull-center\" name=\"ok\" >Ok</button></form>
		<form action=\"\" method=\"post\"><button class=\"btn btn-success btn-large pull-center\" name=\"del\" onClick='return delete_test()' >Delete</button>
		</form>";
		
		if(isset($_POST["del"])){
				$del=mysql_query("delete from forum where f_id='$id'");
				}	
				}
			else{
			echo "Deleted";
